...
if ($action === "getBooks") {
    ...
    if (mysqli_num_rows($books) == 0) {
        ...
        while ($book = mysqli_fetch_assoc($books)) {
            echo "<li> {$book["title"]} ({$book["id"]}) - {$book["genre"]} - {$book["author_name"]} - ({$book["quantity"]} шт в складе) <button onclick=\"window.location.href='../book_edit.php?id={$book['id']}'\" class='editBook'>Изменить</button> <button id={$book["id"]} class='deleteBook'>Удалить</button></li>";
        }
    }
    exit;
}
